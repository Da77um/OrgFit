...
| `GET /api/v1/staff` | Super Admin keyset page `{items,nextCursor}` (Post-Audit Repair Pass 1; filters `q,role,status`, `limit` 1–100, opaque `cursor`) |
...
| `GET /api/v1/audit` | Super Admin keyset page of sanitized audit records (Post-Audit Repair Pass 1; filters `action,actorId,targetId,organizationId,from,to`) |
| `/health/live`, `/health/ready` | Coarse liveness / config+DB-role+schema readiness, no credentials or diagnostics |

**Superseded by Post-Audit Repair Pass 1 (migration 019):** the revoke-sessions endpoint, the `/staff`, `/audit`, `/settings` and `/profile` screens and keyset pagination now exist; see [api-contracts.md](api-contracts.md) and D-128 … D-135. The rest of this paragraph is the foundation-era record. A complete access replacement currently revokes sessions; the guarded SQL revoke routine also exists for future authorized integration. Own-locale preference is an idempotent metadata replacement without revision/idempotency headers. The general Phase 01 CRUD contracts apply when those remaining endpoints are implemented. Lists are foundation previews bounded at 100; Phase 03 must implement honest cursor pagination for its organization list rather than copying the preview response unchanged.
