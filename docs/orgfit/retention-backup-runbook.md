...
| `npm run tombstones:ship` | **at least as often as the backup RPO** (proposed: every 5 min) | deletions since the last ship could be resurrected by a restore |
...
| Tombstone ledger | deletion records | **outside every database backup**; append-only (object lock where available); retained ≥ 36 days |
...
5. **Replay the ledger:** `npm run restore:reapply`. It purges anonymous campaigns that were purged, re-expires attachments, reports and exports, re-destroys campaign keys, erases intake that had been erased — **only where** the anonymous marker exists, the campaign was below threshold, or its output was itself purged — then runs a retention pass and clears the gate.
6. **If it exits 2**, it found an incident (§4 of the incident runbook): accepted envelopes whose anonymous output the restore lost, for a campaign whose intake had already been erased. The envelopes are kept and the environment stays closed. Humans decide; `--open-despite-incidents` records that decision.
